...
p <- ggplot(plot_df, aes(x, y)) +
  ...
  geom_vline(
    data = plot_df |> filter(row == 3, col %in% c(6, 7)),
    xintercept = 0,
    linetype = "dashed",
    color = "grey45",
    linewidth = 0.3
  ) +
  facet_wrap(~ panel, nrow = 3, scales = "free_y", labeller = labeller(panel = label_map)) +
  theme_void(base_size = 11) +
  theme(
    strip.text = element_text(size = 8.5, face = "plain", margin = margin(b = 2)),
