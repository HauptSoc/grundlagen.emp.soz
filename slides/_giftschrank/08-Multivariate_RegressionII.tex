...
\subtitle{Multivariate Regression - Komplexere Fälle}
...
{{Ein Collider liegt dann vor, wenn ein Treatment X einen Einfluss auf das Outcome Y hat, aber parallel die Verteilung einer dritten Eigenschaft Z vom Outcome bedingt wird.  In einem solchen Fall führt die Isolation des Einflusses von X $\rightarrow$ Y mit Hilfe von Z zu völlig falschen Ergebnissen.}}

\end{frame}


\begin{frame}{Medienkonsum und Übergewicht (again)}
\begin{columns}
...
\begin{itemize}
	\item Selbstverletzungstendenzen bilden in Modell d ein Collider mit Übergewicht und Medienkonsum. 
	\item Laut diesem Modell neigen Personen mit Übergewicht stärker zu Selbstverletzungen und sind daher in der Gruppe der ``Selbstverletztenden'' überrepräsentiert. 
	\item Das Modell nimmt außerdem einen direkten Einfluss des Medienkonsums auf die Selbstverletzungstendenz an. 
	\item Wenn wir den Einfluss von Medienkonsum auf Übergewicht isolieren wollen, dürfen wir nicht für Selbstverletzungstendenzen kontrollieren, weil die Unterscheidung nach diesen Tendenzen \textit{eine implizite, erneute} Unterscheidung nach Übergewicht ist.
\end{itemize}
\end{column}
...
\begin{frame}{Fördern Bibliotheksbesuche das Leseinteresse?}
\begin{columns}
...
\begin{column}{.6\linewidth}
\begin{itemize}
	\item Im linken Modell gehen Schüler:innen mit höherem Leseinteresse mit höherer Wahrscheinlichkeit in die Bibliothek. 
	\item Frauen gehen mit höherer Wahrscheinlichkeit in die Bibliothek. 
	\item In diesem Modell erhöht ein Bibliotheksbesuch das Leseinteresse nicht!
\end{itemize}
\end{column}
...
\end{center}
{{\scriptsize Source: \href{https://www.aerzteblatt.de/archiv/223244/Collider-Bias-in-Beobachtungsstudien-Konsequenzen-fuer-die-medizinische-Forschung}{Tönnies et al. (2022)}}}
\end{column}

\begin{column}{.5\linewidth}
\begin{itemize}
	\item Die Probleme ``Kontrolle mit einem Collider'' und Stichprobenselektion können sich überlagern. 
	\item Prozesse, die mit einer Stichprobenselektion einhergehen, können auch mit dem Treatment, einer Kontrollvariable und/oder dem Outcome korreliert sein.
	\item In der Grafik bildet einen Fall ab, in der Personen mit geringeren depressiven Zuständen und besserer Gesundheit eher an einer Studie teilnehmen. Dadurch ist die analytische Stichprobe im Vergleich zur Population selektiv. Die Eigenschaft ``Studienteilnahme'' ist daher ein Collider.
\end{itemize}
\end{column}
...
\begin{column}{.5\linewidth}
\begin{itemize}
	\item Soziale Prozesse können weitaus komplexer sein, als Dreiecksbeziehungen. 
	\item Typischerweise können Sie aber jedes einzelne Element in komplexen Beziehungen in Form von Dreiecksbeziehungen darstellen. 
	\item Reduzieren Sie daher Komplexität, indem Sie stets die Ihr Ziel im Auge behalten: Sie wollen gute Vergleiche. 
\end{itemize}
\end{column}
